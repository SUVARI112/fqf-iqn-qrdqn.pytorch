...
class CosineEmbeddingNetwork(nn.Module):
    ...
    def __init__(self, num_cosines=64, state_dim=2 * 3 * (2 + 1), noisy_net=False):
        super(CosineEmbeddingNetwork, self).__init__()
        linear = NoisyLinear if noisy_net else nn.Linear

        self.net = nn.Sequential(
            linear(num_cosines, state_dim),
            nn.ReLU()
        )
...
class QuantileNetwork(nn.Module):
    ...
    def __init__(self, num_actions, state_dim=2 * 3 * (2 + 1), dueling_net=False,
                 noisy_net=False):
